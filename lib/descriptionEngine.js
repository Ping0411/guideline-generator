...
const { loadMapping } = require('./rulesMapping');
...
function cxmlContainsHint(cxmlContent, hint) {
  ...
  // For attribute-only hints (e.g. @deliveryDate), match the attribute name directly
  if (hint.startsWith('@')) {
    const attrName = hint.slice(1);
    return cxmlContent.includes(`${attrName}="`);
  }
...
function buildDescription(ruleId, hint, language) {
  const lang = 'en';
  const descriptions = {
    'ASN_REQUIRE_DELIVERY_DATE': {
      en: 'Delivery date is required on ship notice.',
    },
    ...
    'OC_REQUIRE_COMMENT_ON_CHANGE': {
      en: 'Comment at line item level is required when any changes to line item are made.',
    ...
    'INV_REQUIRE_BILL_FROM': {
      en: 'Bill From address is required on invoice.',
      zh: '发票中的账单来源地址为必填项。',
      ja: '請求書の請求元住所は必須です。',
    },
...
async function evaluateRules(docType, parsedRules, cxmlContent, answers) {
  ...
  const JA_FORCED_ADDRESS_RULES = new Set([
    'INV_REQUIRE_BILL_TO',
    'INV_REQUIRE_SOLD_TO',
    'INV_REQUIRE_FROM',
    'INV_REQUIRE_REMIT_TO',
  ...
  for (const ruleDef of rules) {
    ...
    // Japanese + Invoice: force-trigger address rules regardless of Yes/No
    const jaForced = language === 'ja' && docType === 'Invoice' && JA_FORCED_ADDRESS_RULES.has(ruleDef.id);

    const shouldTrigger = jaForced ? true
      : ruleDef.triggerOnBoth ? true
  ...
  for (const { ruleDef, ruleEntry } of triggeredDescRules) {
    ...
    // Extra hint: write an additional description to a parent/sibling element (once per rule, not per hint)
    if (anyMatched && ruleDef.extraHint) {
      const eh = ruleDef.extraHint;
      if (eh.type === 'invoicePartnerFor') {
        descriptionTriggers.push({
          ruleId: ruleDef.id + '_EXTRA',
          ruleText: ruleEntry.rule,
          cxmlElement: `InvoicePartner@_containsRole=${eh.role}`,
          description: `InvoicePartner element for ${eh.label} is mandatory`,
        });
      } else if (eh.type === 'element') {
        descriptionTriggers.push({
          ruleId: ruleDef.id + '_EXTRA',
          ruleText: ruleEntry.rule,
          cxmlElement: eh.name,
          description: eh.label,
        });
      }
    }
  ...
  if (docType === 'OC') {
    const ocDescs = buildOcDescriptions(answers, parsedRules, cxmlContent, language, matchedMap);
    descriptionTriggers.push(...ocDescs);

    // OC reject descriptions (conditional, may produce mergeGroup triggers)
    const ocRejectDescs = buildOcRejectDescriptions(answers, parsedRules, cxmlContent, language, matchedMap);
    descriptionTriggers.push(...ocRejectDescs);

    // OC reject Project Requirements (rule 1: Partially / Fully)
    const ocPartialEntry = matchedMap.get('OC_ALLOW_PARTIAL_REJECT');
    if (ocPartialEntry) {
      const val = (ocPartialEntry.value || '').trim().toLowerCase();
  ...
  // Invoice types derived from transaction rules
  let invoiceTypesFromRules = [];
  if (docType === 'Invoice') {
    invoiceTypesFromRules = await buildInvoiceTypesFromRules(parsedRules, language);
  }

  return { descriptionTriggers, unmatchedTriggers, projectReqTriggers, invoiceTypesFromRules };
}
 ...
 */
function buildOcDescriptions(answers, parsedRules, cxmlContent, language, matchedMap) {
  const triggers = [];
  const lang = 'en';
  ...
  if (answers.q9 === 'Yes') {
    if (cxmlContainsHint(cxmlContent, 'ConfirmationHeader')) {
      ...
      triggers.push({
        ...
        cxmlElement: 'ConfirmationHeader/@type',
        description: headerDesc[lang],
      });
    }

    if (cxmlContainsHint(cxmlContent, 'ConfirmationItem')) {
      triggers.push({
        ruleId: 'OC_ENHANCED_CONFIRMATIONITEM',
        cxmlElement: 'ConfirmationItem',
        description: 'ConfirmationItem element is Mandatory when confirmation type at header is "allDetail".',
      });
    }

    if (cxmlContainsHint(cxmlContent, 'ItemIn')) {
      const itemInDesc = {
        en: 'ItemIn element is required when header confirmation type is "allDetail".',
  ...
  if (answers.q9 === 'No') {
    if (cxmlContainsHint(cxmlContent, 'ConfirmationHeader')) {
      const rejectEntireAllowed = isRuleMatchedAndNo(parsedRules, 'OC_NO_REJECT_ENTIRE_PO', matchedMap);
  ...
  // ── OC_REQUIRE_DELIVERY_DATE: build description with optional tolerance ────
  if (isRuleMatchedAndYes(parsedRules, 'OC_REQUIRE_DELIVERY_DATE', matchedMap) &&
      cxmlContainsHint(cxmlContent, 'ConfirmationStatus/@deliveryDate')) {
    // Scan parsedRules for tolerance entries (multilingual) — identify before/after by sub-rule text
    let beforeDays = NaN;
    let afterDays  = NaN;
    for (const [, entry] of parsedRules) {
      const r = (entry.rule || '').toLowerCase();
      const s = (entry.subrule1 || '').toLowerCase();
      // Match main rule in English / Chinese / Japanese
      const isToleranceRule =
        r.includes('delivery date tolerance') ||
        r.includes('交付日期容限') ||
        r.includes('配達日の許容範囲');
      if (!isToleranceRule) continue;
      const isBefore =
        s.includes('before') || s.includes('早于') || s.includes('より前') ||
        s.includes('前') || s.includes('早');
      const isAfter =
        s.includes('after') || s.includes('晚于') || s.includes('より後') ||
        s.includes('後') || s.includes('晚');
      const parsed = entry.value ? parseInt(entry.value, 10) : NaN;
      if (isBefore && !isNaN(parsed)) beforeDays = parsed;
      if (isAfter  && !isNaN(parsed)) afterDays  = parsed;
    }
    const hasBefore = !isNaN(beforeDays);
    const hasAfter  = !isNaN(afterDays);

    let desc;
    if (hasBefore && hasAfter) {
      desc = `Delivery date is required on order confirmation, with a tolerance of up to ${beforeDays} days before and up to ${afterDays} days after the requested delivery date.`;
    } else if (hasBefore) {
      desc = `Delivery date is required on order confirmation, with a tolerance of up to ${beforeDays} days before the requested delivery date.`;
    } else if (hasAfter) {
      desc = `Delivery date is required on order confirmation, with a tolerance of up to ${afterDays} days after the requested delivery date.`;
    } else {
      desc = 'Delivery date is required on order confirmation.';
    }

    triggers.push({
      ruleId: 'OC_REQUIRE_DELIVERY_DATE',
      cxmlElement: 'ConfirmationStatus/@deliveryDate',
      description: desc,
    });
  }

  // ── Past date check: only when OC_NO_PAST_DATES=Yes AND the relevant date rule is also Yes ──
  const noPastDates = (() => {
    for (const [, entry] of parsedRules) {
      if (!entry.rule) continue;
      const r = entry.rule.toLowerCase();
      if ((r.includes('dates in the past') && r.includes('order confirmation')) ||
          r.includes('提交订单确认时允许选择过去的预计发货或交付日期') ||
          r.includes('オーダー確認が提出される際、出荷予定日または配達予定日に過去の日付')) {
        return entry.isYes === true;
      }
    }
    return false;
  })();
  if (noPastDates) {
    const pastDateDesc = 'Past date is not allowed.';
    if (isRuleMatchedAndYes(parsedRules, 'OC_REQUIRE_SHIP_DATE', matchedMap) &&
        cxmlContainsHint(cxmlContent, 'ConfirmationStatus/@shipmentDate')) {
      triggers.push({
        ruleId: 'OC_NO_PAST_DATES_SHIP',
        cxmlElement: 'ConfirmationStatus/@shipmentDate',
        description: pastDateDesc,
      });
    }
    if (isRuleMatchedAndYes(parsedRules, 'OC_REQUIRE_DELIVERY_DATE', matchedMap) &&
        cxmlContainsHint(cxmlContent, 'ConfirmationStatus/@deliveryDate')) {
      triggers.push({
        ruleId: 'OC_NO_PAST_DATES_DELIVERY',
        cxmlElement: 'ConfirmationStatus/@deliveryDate',
        description: pastDateDesc,
      });
    }
 ...
 */
function isRuleMatchedAndNo(parsedRules, ruleId, matchedMap) {
  if (matchedMap) {
    const entry = matchedMap.get(ruleId);
    return entry ? entry.isYes === false : false;
  }
  for (const [, entry] of parsedRules) {
 ...
 */
function buildOcRejectDescriptions(answers, parsedRules, cxmlContent, language, matchedMap) {
  const triggers = [];
  const lang = 'en';
  ...
  if (isRuleMatchedAndYes(parsedRules, 'OC_REQUIRE_REJECT_REASON_HEADER', matchedMap)) {
  ...
  if (isRuleMatchedAndYes(parsedRules, 'OC_REQUIRE_REJECT_REASON_LINE', matchedMap)) {
 ...
 */
function isRuleMatchedAndYes(parsedRules, ruleId, matchedMap) {
  if (matchedMap) {
    const entry = matchedMap.get(ruleId);
    return entry ? entry.isYes === true : false;
  }
  for (const [, entry] of parsedRules) {
    if (entry.matchedId === ruleId) {
      console.log(`[isRuleMatchedAndYes] ${ruleId}: rule="${(entry.rule||'').slice(0,50)}" isYes=${entry.isYes}`);
      return entry.isYes === true;
    }
  }
  return false;
...
function buildPoDescriptions(answers, parsedRules, cxmlContent, language) {
  ...
  if (q5 && cxmlContainsHint(cxmlContent, 'ShipTo')) {
    ...
    const shipToDesc = {
      en: isHeader ? 'ShipTo address information is sent on header level.'
        : isLine   ? 'ShipTo address information is sent on line level.'
        : 'ShipTo address information could be sent on header or line level.',
  ...
  if ((q7 === 'Yes' || q7 === 'Optional') && cxmlContainsHint(cxmlContent, 'Tax')) {
    const taxDesc = {
      en: 'Tax Element: Tax is mandatory for all POs.',
      zh: '所有采购订单均需提供税务信息。',
      ja: 'すべての発注書に税情報が必須です。',
    };
    triggers.push({
...
function checkRuleIsNo(parsedRules, canonicalText) {
  for (const [, entry] of parsedRules) {
    if (!entry.rule) continue;
    const r = entry.rule.toLowerCase();
    if (r.includes('omit payment terms') ||
        r.includes('省略付款条款') ||
        r.includes('支払条件を省略')) {
      return !entry.isYes;
    }
...
/**
 * Derive supported invoice types from transaction rules.
 * Returns an array of invoice type strings (English labels).
 */
async function buildInvoiceTypesFromRules(parsedRules, language) {
  const types = [];

  // Load mapping for non-English rule text lookup
  const mapping = language !== 'en' ? await loadMapping() : null;
  const lookup = mapping
    ? (language === 'zh' ? mapping.enToZh : mapping.enToJa)
    : null;

  // Translate an English rule text to the user's language for matching parsedRules
  function tr(enText) {
    if (!lookup) return enText;
    return lookup.get(enText.toLowerCase().replace(/\.$/, '')) || enText;
  }

  // Check if a rule (by English text, optionally with subrule1) is Yes in parsedRules
  function isYes(enText, sub1 = '') {
    const target = tr(enText).toLowerCase();
    for (const [, entry] of parsedRules) {
      if (!entry.rule) continue;
      const ruleMatch = entry.rule.toLowerCase().includes(target) ||
                        target.includes(entry.rule.toLowerCase().replace(/\.$/, ''));
      if (!ruleMatch) continue;
      if (sub1) {
        const subTarget = tr(sub1).toLowerCase();
        const subMatch = (entry.subrule1 || '').toLowerCase().includes(subTarget) ||
                         subTarget.includes((entry.subrule1 || '').toLowerCase().replace(/\.$/, ''));
  ...
  if (isYes('Allow suppliers to send header level credit memos'))
    types.push('Header Level Credit Memo');

  if (isYes('Allow suppliers to send invoices with a contract reference'))
    types.push('Contract Invoice');

  if (isYes('Allow suppliers to send invoices with service information'))
    types.push('Service Invoice');

  if (isYes('Allow suppliers to send non-PO invoices'))
    types.push('Non-PO Invoice');

  if (isYes('Allow suppliers to send line-item debit memo'))
    types.push('Line Level Debit Memo');

  if (isYes('Allow suppliers to send header credit memos and debit memos')) {
    if (!types.includes('Header Level Credit Memo')) types.push('Header Level Credit Memo');
    types.push('Header Level Debit Memo');
  }

  // Line-item credit memo sub-rules
  if (isYes('Allow suppliers to send line-item credit memo', 'Quantity Adjustment'))
    types.push('Quantity Adjustment Line Level Credit Memo');

  if (isYes('Allow suppliers to send line-item credit memo', 'Price Adjustment'))
    types.push('Price Adjustment Line Level Credit Memo');

  if (isYes('Allow suppliers to send line-item credit memo with quantity adjustment'))
    types.push('Quantity Adjustment Line Level Credit Memo');

  if (isYes('Allow suppliers to send line-item credit memo with price adjustment'))
    types.push('Price Adjustment Line Level Credit Memo');

  if (isYes('Allow suppliers to send line-item debit memo with price adjustment'))
    types.push('Price Adjustment Line Level Debit Memo');
